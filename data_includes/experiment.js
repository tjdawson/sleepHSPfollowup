var manualSendResults = true; 
var shuffleSequence = seq("intro", "intro1", "sep", 
"test_zant_come_highest","sep",
"test_telpen_bag_distractor","sep",
"test_tula_mommy_target","sep",
"test_vash_shoe_target","sep",
"test_telpen_throw_highest","sep",
"test_bist_stop_lowest","sep",
"test_zant_pillow_distractor","sep",
"test_jair_pet_distractor","sep",
"test_tula_stop_lowest","sep",
"test_zant_book_target","sep",
"test_telpen_move_lowest","sep",
"test_zant_here_lowest","sep",
"test_tace_phone_target","sep",
"test_doon_dog_distractor","sep",
"test_blime_give_distractor","sep",
"test_blime_look_lowest","sep",
"test_vash_pantry_distractor","sep",
"test_jair_pantry_highest","sep",
"test_tiz_thing_target","sep",
"test_tula_his_distractor","sep",
"test_vash_elephant_highest","sep",
"test_doon_his_highest","sep",
"test_tace_look_lowest","sep",
"test_blime_bag_highest","sep",
"test_bist_throw_distractor","sep",
"test_mipen_bag_target","sep",
"test_tiz_hit_highest","sep",
"test_tace_pillow_highest","sep",
"test_tiz_thanks_lowest","sep",
"test_bist_dog_highest","sep",
"test_blime_toy_target","sep",
"test_tace_hit_distractor","sep",
"test_doon_necklace_target","sep",
"test_geck_pet_highest","sep",
"test_doon_baby_lowest","sep",
"test_geck_phone_distractor","sep",
"test_tiz_elephant_distractor","sep",
"test_geck_nose_target","sep",
"test_telpen_ball_target","sep",
"test_mipen_come_distractor","sep",
"test_mipen_cupboard_lowest","sep",
"test_bist_horse_target","sep",
"test_vash_down_lowest","sep",
"test_jair_good_lowest","sep",
"test_geck_wipe_lowest","sep",
"test_jair_time_target","sep",
"test_tula_phone_highest","sep",
"test_mipen_give_highest","outro1","sr","outro2"
);

var defaults = [
        "Separator", {
            transfer: "keypress",
            normalMessage: "Press any key to continue.",
            errorMessage: "Press any key to continue."
        },
        "DashedSentence", {
            mode: "self-paced reading"
        },
        "AcceptabilityJudgment", {
            as: ["1", "2", "3", "4", "5", "6", "7"],
            presentAsScale: true,
            instructions: "Use number keys or click boxes to answer.",
            leftComment: "(Bad)", rightComment: "(Good)"
        },
        "Question", {
            hasCorrect: true
        },
        "Message", {
            hideProgressBar: true
        },
        "Form", {
            hideProgressBar: false,
            continueOnReturn: true,
            saveReactionTime: false
        }
    ];

var items = [    ["sr", "__SendResults__", { }],
["sep", "Separator", { }],
        
        //
        
        ["intro", "Form", {
            html: {include: "intro.html"},
            hideProgressBar: true,
            countsForProgressBar: false
        
        }],
        
        //
        
        ["intro1", "Form", {
            html: {include: "intro1.html"},
            hideProgressBar: true,
            countsForProgressBar: false,
    validators: {
    audiotest: function(s)
    { if (s == "3401")
    return true; else return "Incorrect entry for \u2018test numbers\u2019";}
    }
        
        }],
        
        //
        
        ["outro1", "Form", {
            html: {include: "outro1.html"},
            hideProgressBar: true,
            countsForProgressBar: false
        
        }],
        
        //
        
        ["outro2", "Form", {
            html: { include: "outro2.html" },
            hideProgressBar: true,
            countsForProgressBar: false,
        } ],
        
        //
["test_tula_mommy_target", "Form", { 
html: {include: "test_tula_mommy_target.html"} 
}],

["test_tula_phone_highest", "Form", { 
html: {include: "test_tula_phone_highest.html"} 
}],

["test_tula_stop_lowest", "Form", { 
html: {include: "test_tula_stop_lowest.html"} 
}],

["test_tula_his_distractor", "Form", { 
html: {include: "test_tula_his_distractor.html"} 
}],

["test_vash_shoe_target", "Form", { 
html: {include: "test_vash_shoe_target.html"} 
}],

["test_vash_elephant_highest", "Form", { 
html: {include: "test_vash_elephant_highest.html"} 
}],

["test_vash_down_lowest", "Form", { 
html: {include: "test_vash_down_lowest.html"} 
}],

["test_vash_pantry_distractor", "Form", { 
html: {include: "test_vash_pantry_distractor.html"} 
}],

["test_geck_nose_target", "Form", { 
html: {include: "test_geck_nose_target.html"} 
}],

["test_geck_pet_highest", "Form", { 
html: {include: "test_geck_pet_highest.html"} 
}],

["test_geck_wipe_lowest", "Form", { 
html: {include: "test_geck_wipe_lowest.html"} 
}],

["test_geck_phone_distractor", "Form", { 
html: {include: "test_geck_phone_distractor.html"} 
}],

["test_tiz_thing_target", "Form", { 
html: {include: "test_tiz_thing_target.html"} 
}],

["test_tiz_hit_highest", "Form", { 
html: {include: "test_tiz_hit_highest.html"} 
}],

["test_tiz_thanks_lowest", "Form", { 
html: {include: "test_tiz_thanks_lowest.html"} 
}],

["test_tiz_elephant_distractor", "Form", { 
html: {include: "test_tiz_elephant_distractor.html"} 
}],

["test_bist_horse_target", "Form", { 
html: {include: "test_bist_horse_target.html"} 
}],

["test_bist_dog_highest", "Form", { 
html: {include: "test_bist_dog_highest.html"} 
}],

["test_bist_stop_lowest", "Form", { 
html: {include: "test_bist_stop_lowest.html"} 
}],

["test_bist_throw_distractor", "Form", { 
html: {include: "test_bist_throw_distractor.html"} 
}],

["test_jair_time_target", "Form", { 
html: {include: "test_jair_time_target.html"} 
}],

["test_jair_pantry_highest", "Form", { 
html: {include: "test_jair_pantry_highest.html"} 
}],

["test_jair_good_lowest", "Form", { 
html: {include: "test_jair_good_lowest.html"} 
}],

["test_jair_pet_distractor", "Form", { 
html: {include: "test_jair_pet_distractor.html"} 
}],

["test_tace_phone_target", "Form", { 
html: {include: "test_tace_phone_target.html"} 
}],

["test_tace_pillow_highest", "Form", { 
html: {include: "test_tace_pillow_highest.html"} 
}],

["test_tace_look_lowest", "Form", { 
html: {include: "test_tace_look_lowest.html"} 
}],

["test_tace_hit_distractor", "Form", { 
html: {include: "test_tace_hit_distractor.html"} 
}],

["test_doon_necklace_target", "Form", { 
html: {include: "test_doon_necklace_target.html"} 
}],

["test_doon_his_highest", "Form", { 
html: {include: "test_doon_his_highest.html"} 
}],

["test_doon_baby_lowest", "Form", { 
html: {include: "test_doon_baby_lowest.html"} 
}],

["test_doon_dog_distractor", "Form", { 
html: {include: "test_doon_dog_distractor.html"} 
}],

["test_telpen_ball_target", "Form", { 
html: {include: "test_telpen_ball_target.html"} 
}],

["test_telpen_throw_highest", "Form", { 
html: {include: "test_telpen_throw_highest.html"} 
}],

["test_telpen_move_lowest", "Form", { 
html: {include: "test_telpen_move_lowest.html"} 
}],

["test_telpen_bag_distractor", "Form", { 
html: {include: "test_telpen_bag_distractor.html"} 
}],

["test_blime_toy_target", "Form", { 
html: {include: "test_blime_toy_target.html"} 
}],

["test_blime_bag_highest", "Form", { 
html: {include: "test_blime_bag_highest.html"} 
}],

["test_blime_look_lowest", "Form", { 
html: {include: "test_blime_look_lowest.html"} 
}],

["test_blime_give_distractor", "Form", { 
html: {include: "test_blime_give_distractor.html"} 
}],

["test_zant_book_target", "Form", { 
html: {include: "test_zant_book_target.html"} 
}],

["test_zant_come_highest", "Form", { 
html: {include: "test_zant_come_highest.html"} 
}],

["test_zant_here_lowest", "Form", { 
html: {include: "test_zant_here_lowest.html"} 
}],

["test_zant_pillow_distractor", "Form", { 
html: {include: "test_zant_pillow_distractor.html"} 
}],

["test_mipen_bag_target", "Form", { 
html: {include: "test_mipen_bag_target.html"} 
}],

["test_mipen_give_highest", "Form", { 
html: {include: "test_mipen_give_highest.html"} 
}],

["test_mipen_cupboard_lowest", "Form", { 
html: {include: "test_mipen_cupboard_lowest.html"} 
}],

["test_mipen_come_distractor", "Form", { 
html: {include: "test_mipen_come_distractor.html"} 
}],

["end", "Form", {
            html: {include: "end.html"},
            hideProgressBar: true,
            countsForProgressBar: false
        
        }]
    ];